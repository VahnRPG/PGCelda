...
class ImprovedNoise {
	...
	*/
	 ...
	 */
	public function fBm($x, $y, $z, $H, $octaves) {
		$answer = 0;
		for($i = 0; $i < $octaves; $i++) {
			$answer = $answer + $this->noise($x, $y, $z) / (1 << $H * $i);
			$x = $x * 2;
			$y = $y * 2;
			$z = $z * 2;
		}
	 ...
	 * a lacunarity of two.  This simplification is done for speed.
	 */
	public function turbulance($x, $y, $z, $octaves) {
		$frequency = 1;
		$t = 0;
		for($octave = 0; $octave < $octaves; $octave++) {
			$t = $t + abs($this->noise($x, $y, $z)) / $frequency;
			$x = $x * 2;
			$y = $y * 2;
			$z = $z * 2;
	 ...
	 */
	public function wood($x, $y, $z) {
		$noise = $this->noise($x, $y, $z) * 15.0;
	 ...
	 */
	public function marble($x, $y, $z, $octaves) {
		$t = $this->turbulance($x, $y, $z, $octaves);
		$marble = sin($y + $t);
	 ...
	 */
	public function ridged($x, $y, $z, $octaves, $offset, $gain) {
		$answer = 0;
		$weight = 1.0;
		$frequency = 1;
		
		for($octave = 0; $octave < $octaves; $octave++) {
			$signal = $offset - abs($this->noise($x, $y, $z));
