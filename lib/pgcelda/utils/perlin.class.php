...
class Perlin {
	public $p, $permutation, $seed;
	public $_default_size = 64;
	...
	//This function determines what cube the point passed resides in and determines its value.
	public function smoothNoise($x, $y, $z) {
		...
		$result = $this->lerp(
			$w,
			$this->lerp($v, $this->lerp($u, $this->grad($this->p[$AA], $x, $y, $z),	// AND ADD
	...
	//*
	public function improvedNoise($x, $y, $z, $size=NULL) {
		$x = $x / $size;
		$y = $y / $size;
		$z = $z / $size;
		$X1 = (int) floor($x) & 255;			// FIND UNIT CUBE THAT
	 ...
	 */
	public function fBm($x, $y, $z, $smooth, $H, $octaves) {
		$answer = 0;
		for($i = 0; $i < $octaves; $i++) {
			$answer = $answer + $this->improvedNoise($x, $y, $z, $smooth) / (1 << $H * $i);
			$x = $x * 2;
			$y = $y * 2;
			$z = $z * 2;
		}
	 ...
	 * a lacunarity of two.  This simplification is done for speed.
	 */
	public function turbulance($x, $y, $z, $smooth, $octaves) {
		$frequency = 1;
		$t = 0;
		for($octave = 0; $octave < $octaves; $octave++) {
			$t = $t + abs($this->improvedNoise($x, $y, $z, $smooth)) / $frequency;
			$x = $x * 2;
			$y = $y * 2;
			$z = $z * 2;
	 ...
	 */
	public function wood($x, $y, $z, $smooth) {
		$noise = $this->improvedNoise($x, $y, $z, $smooth) * 15.0;
		$grain = $noise - floor($noise);
		
		return $grain;
	}
	
	/**
	 * Based on the classic (circa 1985) Perlin implementation for generating a marble pattern.
	 */
	public function marble($x, $y, $z, $smooth, $octaves) {
		$t = $this->turbulance($x, $y, $z, $smooth, $octaves);
		$marble = sin($y + $t);
	 ...
	 */
	public function ridged($x, $y, $z, $smooth, $octaves, $offset, $gain) {
		$answer = 0;
		$weight = 1.0;
		$frequency = 1;
		
		for($octave = 0; $octave < $octaves; $octave++) {
			$signal = $offset - abs($this->improvedNoise($x, $y, $z, $smooth));
	...
	public function random1D($x, $size=NULL) {
		if (is_null($size)) {
			$size = $this->_default_size;
		}
	...
	public function random2D($x, $y, $size=NULL) {
		if (is_null($size)) {
			$size = $this->_default_size;
		}
	...
	//Same as random1D() only for 3 dimensions.
	public function random3D($x, $y, $z, $size=NULL) {
		if (is_null($size)) {
			$size = $this->_default_size;
		}
		...
		while($size >= 1) {
			$value += $this->smoothNoise($x * 3 / $size, $y * 3 / $size, $z * 3 / $size);
			$size--;
		}
